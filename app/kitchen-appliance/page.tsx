...
import { applianceCategories } from '@/lib/menuData';
...
export default function KitchenAppliancePage() {
  ...
  const allAppliances = useMemo(() => {
    return applianceCategories.flatMap(c => c.items.map(item => ({ ...item, categoryName: c.name })));
  }, []);

  const highestPrice = useMemo(() => {
    return Math.max(...allAppliances.map(item => item.price), 100);
  }, [allAppliances]);

  const [maxPrice, setMaxPrice] = useState(highestPrice);

  const filteredItems = useMemo(() => {
    return allAppliances.filter(item => {
      const matchCat = selectedCat === 'All' || item.categoryName === selectedCat;
      const matchPrice = item.price <= maxPrice;
      return matchCat && matchPrice;
    });
  }, [selectedCat, maxPrice, allAppliances]);
    ...
    <main className="min-h-screen overflow-x-hidden bg-charcoal">
      ...
      <div className="pt-[140px] pb-24 px-6 lg:px-14 max-w-7xl mx-auto min-h-[70vh]">
        <div className="text-center mb-10">
          <p className="text-orange font-semibold uppercase tracking-wider mb-2 text-sm">Smart Tools</p>
          <h1 className="text-5xl font-serif font-medium text-warm-white mb-4">Kitchen & Appliance</h1>
          <p className="text-warm-white/60 max-w-xl mx-auto">Equip your home with the very best small appliances, smart gadgets, and professional utensils.</p>
        </div>
        ...
        <div className="w-full relative z-10">
          {filteredItems.length === 0 ? (
            <div className="text-center py-32 glass rounded-[32px] flex flex-col items-center justify-center">
              <p className="text-warm-white/60 text-lg mb-6">No tools found matching your filters.</p>
              <button 
                onClick={() => { setSelectedCat('All'); setMaxPrice(highestPrice); }}
